...
An implementation SHOULD publish its conformance claim in the machine-readable form described in §18.8. An implementation MUST NOT declare a Profile it does not fully satisfy.
...
## 18.7 Composition Profile *(since v1.4)*

The Composition Profile covers rendering definitions sourced from origins other than the one serving the document — [`01_Core_Concepts.md`](01_Core_Concepts.md) §1.9 (`DefinitionSource`), the [`view`](02_Widgets.md#2131-view-since-v14) widget, [`06_Runtime_Contract.md`](06_Runtime_Contract.md) §6.11 (resolution), [`07_Security.md`](07_Security.md) §7.10 (origin isolation), and [`08_Client_Extensions.md`](08_Client_Extensions.md) §8.8 (cross-origin scope).

This profile is the conformance gate for multi-origin composition. Per §1.7.3 a runtime's support surface is declared by profile, not by version; a host that cannot isolate scopes per origin simply does not claim this profile.

### 18.7.1 Required

Runtimes claiming the Composition Profile MUST:

- Accept all four `DefinitionSource` forms (§1.9.1), including the qualified `{ $ref, from }` reference and the binding form.
- Accept `Origin` of the form `{ "connection": <id> }`, resolving a binding inside it in the scope containing the source (§6.11.2).
- **Reject** an `Origin` whose key they do not recognise, and reject a `from` naming a connection that does not exist — never falling back to the current origin (§6.11.2, §7.10.1 rule 6).
- Implement the `view` widget with `source`, `props`, `fallback`, `loading`, `onError`, and `theme` (§2.13.1).
- Accept a `DefinitionSource` anywhere `RouteValue` is accepted (§1.2.1).
- Run each embedded definition in its own scope — own state tree, own subscription registry, own permission and storage identity (§6.11.3).
- Enforce every requirement in [`07_Security.md`](07_Security.md) §7.10.1, in particular permission **intersection** (never union) and `props` as the only embedder→embedded channel.
- Dispatch `tool` calls, resource reads, and subscriptions inside an embedded scope to that scope's ambient origin (§1.9.5).
- Route `notifications/resources/updated` to the scope(s) bound to the connection that delivered them, never to the embedder (§6.11.3).
- Contain failure: an unresolvable source renders `fallback` and fires `onError` without failing sibling views or the embedding page (§6.11.4).
- Re-resolve and remount an embedded scope when its connection reconnects (§6.11.4).
- Enforce a maximum embedding depth and detect origin cycles, failing the offending source rather than recursing (§6.11.4).

### 18.7.2 Optional

- Explicit cross-scope actions carrying an `origin` (§8.8.3). A runtime that does not implement them MUST reject such actions rather than dispatch them to the ambient origin.
- `theme: "own"` (§2.13.1). A runtime that supports only `inherit` MUST say so in its conformance claim; it MUST NOT silently substitute one for the other, since the choice is visible to the user.

### 18.7.3 Without the Composition Profile

A runtime that does not claim this profile:

- MUST treat `"type": "view"` as an unknown widget type per §18.2.1 (error placeholder, no crash).
- MUST reject a `DefinitionSource` carrying `from`, and a `RouteValue` of the qualified form — it MUST NOT resolve `$ref` against its own origin. This is the one place where the forward-compatibility allowance of §1.7.2 does not apply, because a permissive reading substitutes one server's UI for another's.

Documents that compose origins therefore degrade to a visible error on a non-composing runtime, never to a silently wrong render.

---

## 18.8 Conformance Claims
...
## 18.10 Informative: Feature-to-Profile Map
...
| [`01_Core_Concepts.md`](01_Core_Concepts.md) §1.9 — DefinitionSource / Origin | Composition (v1.4+) |
| [`02_Widgets.md`](02_Widgets.md) §2.13.1 — `view` | Composition (v1.4+) |
| [`06_Runtime_Contract.md`](06_Runtime_Contract.md) §6.11 — Multi-origin resolution | Composition (v1.4+) |
| [`07_Security.md`](07_Security.md) §7.10 — Origin isolation | Composition (v1.4+) |
| [`08_Client_Extensions.md`](08_Client_Extensions.md) §8.8 — Cross-origin scope | Composition (v1.4+) |
