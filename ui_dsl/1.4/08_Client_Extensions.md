...
## 8.8 Cross-Origin Scope *(since v1.4, Composition Profile)*

§8.4 (Permission System), §8.3 (`client://` storage), and §8.7 (revocation) are all written against **"the current MCP server identity"**. v1.4 lets one document render definitions from several origins ([`01_Core_Concepts.md`](01_Core_Concepts.md) §1.9). This section states how those rules apply when more than one identity is in play.

**The existing rules are unchanged.** They are not rewritten to be plural. Instead, each embedded scope carries its own server identity, and every rule in §8.3 / §8.4 / §8.7 applies within that scope exactly as written. In particular, *"Storage is scoped per MCP server identity; cross-server reads are prohibited"* (§8.3) remains literally true — there are simply several scopes alive at once, none of which can read another's.

### 8.8.1 What is keyed to which identity

| Concern | Identity used |
|---------|---------------|
| `client://` storage namespace | The **embedded** scope's origin. |
| Permission grants and their persistence | The **embedded** scope's origin. |
| Permission revocation (§8.7) | Revokes for one origin; other origins in the same document are unaffected. |
| Channel permissions (§8.6) | The origin of the scope that declared the channel. |
| `tool` / resource dispatch | The ambient origin of the enclosing scope ([`01_Core_Concepts.md`](01_Core_Concepts.md) §1.9.5). |

### 8.8.2 Effective permissions are an intersection

For an embedded scope with origin *O*:

```
effective(O) = granted(embedder → O)  ∩  requested(embedded definition)
```

Embedding never elevates. A definition that would prompt for `file.write` when run standalone does not receive it because its embedder holds it; the embedder's grant toward *O* bounds it from above, and the definition's own declaration bounds it from below.

A host MAY prompt the user per origin on first composition, exactly as it prompts on first standalone use.

### 8.8.3 Explicit cross-scope actions

An action MAY carry an explicit `origin` to dispatch outside its ambient scope. This is a distinct capability, **denied by default**, and a runtime that does not implement it MUST reject the action rather than dispatch it to the ambient origin. It exists for deliberate orchestration (a controller view acting on a sensor); it is not the normal path, which is §1.9.5's ambient scoping.
