...
### 2.13.1 `view` *(since v1.4)*

Embeds a definition sourced from anywhere — including another MCP origin — at this point in the tree.

`view` is the **consumer** side of composition. [`11_Bundle_Metadata.md`](11_Bundle_Metadata.md) §11.9 `dashboard` declares how an application presents itself *when embedded*; `view` is how an application *embeds*. Before v1.4 only the provider side existed.

| Property | Type | Required | Default | Description |
|----------|------|----------|---------|-------------|
| `source` | [`DefinitionSource`](01_Core_Concepts.md#19-definitionsource-since-v14) | yes | — | What to render. Inline definition, a `ui://` URI on the current origin, a qualified `{ $ref, from }` reference to another origin, or a binding to a definition held in state. |
| `props` | object | no | `{}` | Values merged into the embedded definition's initial state. The **only** channel from embedder to embedded (§7.10). One-way. |
| `fallback` | Widget | no | — | Rendered when the source cannot be resolved. Absent = a generic unavailable indicator. |
| `loading` | Widget | no | — | Rendered while the source resolves. |
| `onError` | Action | no | — | Fired with `{{event.error}}` when resolution fails. |
| `theme` | `"inherit"` \| `"own"` | no | `"inherit"` | Whether the embedded definition renders under the embedder's theme or its own. See below. |

**Origin scope.** When `source` names an origin, that origin becomes the ambient origin for the embedded subtree — its `tool` calls, resource reads, subscriptions, state, storage, and permissions all scope to it. This is normative; see [`01_Core_Concepts.md`](01_Core_Concepts.md) §1.9.5, [`07_Security.md`](07_Security.md) §7.10, [`08_Client_Extensions.md`](08_Client_Extensions.md) §8.8.

**Failure is per-view.** An unresolvable source renders that view's `fallback` and fires `onError`; sibling views and the embedding page render normally. A composed screen of four devices with one offline shows three live tiles and one fallback — never a blank page.

**Theme.** `inherit` (default) renders the embedded definition under the embedder's theme so a composed screen reads as one product; this is the right default for summary tiles. `own` honours the embedded definition's own theme, which is appropriate when embedding a full third-party application whose identity should be preserved.

**Nesting.** An embedded definition may itself contain `view`. Runtimes MUST bound the depth and MUST detect origin cycles (A embedding B embedding A); on either, the offending `view` renders its `fallback`. See [`18_Conformance.md`](18_Conformance.md) §18.7.
...
```json
{
  ...
  "columns": 2,
  "children": [
    { "type": "view", "source": { "$ref": "ui://views/summary", "from": { "connection": "{{conn.temp}}" } } },
    { "type": "view", "source": { "$ref": "ui://views/summary", "from": { "connection": "{{conn.humid}}" } } },
    {
      "type": "view",
      "source": { "$ref": "ui://app", "from": { "connection": "{{conn.ctrl}}" } },
      "theme": "own",
      "fallback": { "type": "text", "text": "Controller offline" }
    }
  ]
}
```
